...
export function SignupPage() {
  ...
  async function handleSubmit(event) {
    ...
    setSubmitting(false);
    setStatus(error ? error.message : 'Cuenta creada. Revisa tu correo si la confirmacion por correo esta activada.');
  }

  return (
    <section className="mx-auto max-w-lg px-6 py-24 md:px-10">
      <div className="magazine-frame rounded-[2rem] p-8">
        <p className="editorial-kicker">Unete a FabuRose</p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          ...
          <input className="w-full rounded-full border border-ink/10 bg-white px-5 py-4 outline-none focus:border-ink/40" type="email" placeholder="Correo electronico" value={email} onChange={(event) => setEmail(event.target.value)} required />
          <input className="w-full rounded-full border border-ink/10 bg-white px-5 py-4 outline-none focus:border-ink/40" type="password" placeholder="Contrasena" value={password} onChange={(event) => setPassword(event.target.value)} required minLength="6" />
          <label className="flex items-start gap-3 rounded-[1.25rem] bg-white/60 p-4 text-sm leading-6 text-ink/70">
            ...
            Recibir avisos por correo cuando Fabiana publique nuevos recursos o articulos.
          </label>
        ...
        <p className="mt-6 text-sm text-ink/60">Ya tienes cuenta? <Link className="text-ink underline" to="/login">Inicia sesion</Link></p>
      </div>
    </section>
  );
}