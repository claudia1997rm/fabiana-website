  ...
  return (
    <section className="mx-auto max-w-lg px-6 py-24 md:px-10">
      <div className="magazine-frame rounded-[2rem] p-8">
        ...
        <h1 className="mt-4 font-display text-5xl leading-none tracking-[-0.03em] text-ink">Log in</h1>
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          ...
          <input className="w-full rounded-full border border-ink/10 bg-white px-5 py-4 outline-none focus:border-ink/40" type="password" placeholder="Password" value={password} onChange={(event) => setPassword(event.target.value)} required />
        ...
        <p className="mt-6 text-sm text-ink/60">No account yet? <Link className="text-ink underline" to="/signup">Sign up</Link></p>
      </div>
    </section>
  );
}