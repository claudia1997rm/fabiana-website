...
export function HeroSection({ heroData = defaultHero }) {
  return (
    <section className="section-shell pt-4 md:pt-10">
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-10 sm:px-6 md:px-10 md:pb-24 md:pt-14 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-14 lg:pb-32 lg:pt-20">
        <div className="relative z-10 animate-rise">
          ...
          <h1 className="hero-title max-w-4xl text-[3.9rem] leading-[0.88] text-ink min-[390px]:text-[4.55rem] sm:text-[5.9rem] lg:text-[8.15rem] xl:text-[8.9rem]">{heroData.title}</h1>
          <p className="mt-5 max-w-2xl font-serif text-[1.45rem] italic leading-tight text-ink/80 sm:text-2xl md:mt-6 md:text-[2rem]">{heroData.lead}</p>
          <p className="mt-5 max-w-2xl text-[0.95rem] leading-7 text-ink/70 md:mt-6 md:text-lg md:leading-8">{heroData.subtitle}</p>
        ...
        <div className="relative flex justify-center lg:justify-end">
          ...
          <div className="relative z-10 w-full max-w-[24rem] sm:max-w-[32rem] lg:max-w-[35rem]">
            <div className="glass-panel rounded-[1.8rem] p-3 shadow-soft transition duration-700 hover:-translate-y-1 hover:shadow-lavender md:rounded-[2.25rem] md:p-4">
              <img src={heroData.image} alt="Editorial lifestyle para FabuRose" className="h-[24rem] w-full rounded-[1.45rem] object-cover animate-float sm:h-[33rem] md:h-[38rem] md:rounded-[1.75rem]" />
            </div>
            ...
            <div className="magazine-frame absolute -bottom-4 left-3 max-w-[13rem] rounded-[1.35rem] p-4 transition duration-500 hover:-translate-y-1 hover:shadow-lavender sm:-left-4 md:-bottom-6 md:-left-10 md:max-w-[15rem] md:rounded-[1.6rem] md:p-5">
              ...
              <p className="mt-3 font-serif text-lg leading-6 text-ink md:text-xl">{heroData.microNote}</p>
            </div>

            <div className="absolute -right-3 top-8 hidden w-40 overflow-hidden rounded-[1.45rem] border border-white/60 bg-white/75 p-2 shadow-soft backdrop-blur-sm transition duration-700 hover:-translate-y-1 hover:shadow-lavender sm:block md:-right-8 md:top-14 md:w-48 md:rounded-[1.7rem] md:p-3">
              <img src={heroData.imageSecondary} alt="Detalle editorial de moda para FabuRose" className="h-44 w-full rounded-[1.15rem] object-cover md:h-52 md:rounded-[1.35rem]" />
            </div>
          </div>
        </div>
      </div>
