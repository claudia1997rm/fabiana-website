...
export function UniverseSection() {
  return (
    <section id="universo" className="section-shell border-y border-plum/20 bg-lavenderMist/70">
      <div className="orb left-[-90px] top-24 h-64 w-64 bg-lilac/20" />
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-3 lg:grid-rows-[repeat(2,minmax(0,1fr))]">
          {categories.map((category, index) => {
            ...
            return (
              ...
              >
                <div className={`absolute right-0 top-0 h-28 w-28 rounded-full blur-3xl transition duration-700 group-hover:scale-125 ${featured ? 'bg-lilac/20' : 'bg-lavenderMist/65'}`} />
                ...
                <div className="mt-10 flex items-center justify-between gap-5">
                  ...
                  <span className={`h-px flex-1 origin-left transition duration-700 group-hover:scale-x-90 ${featured ? 'bg-white/20' : 'bg-plum/20'}`} />
                  <span className={`inline-flex h-9 w-9 items-center justify-center rounded-full border transition duration-500 group-hover:-translate-y-0.5 group-hover:translate-x-1 ${featured ? 'border-lavender/25 text-lavender group-hover:bg-lavender/10' : 'border-plum/20 text-plum/70 group-hover:bg-lavenderMist'}`}>
                    →
                  </span>
                </div>
              </Link>
