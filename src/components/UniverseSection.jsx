    ...
    <section id="universo" className="section-shell border-y border-plum/25 bg-[linear-gradient(135deg,rgba(248,245,250,0.92),rgba(85,74,104,0.22))]">
      ...
      <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 lg:py-28">
        <SectionHeading
          ...
          title="Seis lÃƒÂ­neas editoriales para una vida mÃƒÂ¡s bella, consciente y magnÃƒÂ©tica"
          description="Elige una puerta de entrada: hÃƒÂ¡bitos, estilo, estÃƒÂ©tica, fotografÃƒÂ­a, astrologÃƒÂ­a o vida consciente. Desde ahÃƒÂ­ puedes leer, descargar y volver a tu propio ritual."
        />
          ...
          {categories.map((category, index) => {
            ...
            return (
              <Link
                ...
                className={`reveal-on-scroll group hover-lift relative block cursor-pointer overflow-hidden rounded-[2rem] border border-ink/10 p-7 transition duration-700 hover:border-plum/25 hover:shadow-lavender focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-plum/25 focus-visible:ring-offset-4 focus-visible:ring-offset-cloud md:p-8 ${featured ? 'bg-[linear-gradient(135deg,#2A2235,#5E2F3F)] text-white lg:row-span-2' : 'bg-lavenderMist/90 text-ink'}`}
              ...
              >
                ...
                <p className={`text-[10px] uppercase tracking-[0.32em] ${featured ? 'text-white/80' : 'text-plum'}`}>
                  {category.accent}
                </p>
                ...
                <div className="mt-10 flex items-center justify-between gap-5">
                  <span className={`text-[10px] uppercase tracking-[0.3em] transition duration-500 group-hover:tracking-[0.34em] ${featured ? 'text-cloud/70' : 'text-ink/55'}`}>
                    Explorar
                  </span>
                  <span className={`h-px flex-1 origin-left transition duration-700 group-hover:scale-x-90 ${featured ? 'bg-white/25' : 'bg-plum/30'}`} />
                  <span className={`inline-flex h-9 w-9 items-center justify-center rounded-full border transition duration-500 group-hover:-translate-y-0.5 group-hover:translate-x-1 ${featured ? 'border-lavender/25 text-lavender group-hover:bg-lavender/10' : 'border-plum/35 text-plum group-hover:bg-lavenderMist'}`}>
                    Ã¢â€ â€™
                  </span>
                </div>
              </Link>
